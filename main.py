...
def get_two_grade_course(session: requests.Session):
    path = './'
    url = f"https://catalog.ustc.edu.cn/api/teach/lesson/list-for-teach/341?access_token={ACCESS_TOKEN}"
    ...
    for course in tqdm.tqdm(data):
        payload = f'{"{"}"codes":["{course["code"]}"],"semester":341{"}"}'
        response = session.post(url, data=payload)
        json_text = json.loads(response.text)
        if (json_text[0]["grading"] == "二分制"):
            print(json_text[0]["name"]["cn"], json_text[0]["grading"])
            with open(path + 'two_grade_course.txt', 'a', encoding='utf-8') as fp:
